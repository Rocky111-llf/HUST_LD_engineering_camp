...
private:
	...
	ldlidar::Points2D lidar_data;



public:
	int set_lidar_data(ldlidar::Points2D lidar_data_in)
	{
	...
	int DataTransform()
	{
		...
		{
			PointXY point_tmp;
			point_tmp.time_interval = lidar_data[i].stamp;
			point_tmp.x = lidar_data[i].distance * cos(lidar_data[i].angle / 180.0 * 3.14159); // mm
			point_tmp.x /= 1000; // m
			point_tmp.y = lidar_data[i].distance * sin(lidar_data[i].angle / 180.0 * 3.14159); // mm
			point_tmp.y /= 1000; // m
